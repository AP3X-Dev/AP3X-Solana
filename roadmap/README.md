...
| **01.5** | [Solana webhook ingestion](./01.5-solana-webhooks.md) | 1-2 | `@ap3x/solana-webhooks` 0.1 alpha shipped: outbox-first HTTP receiver (HMAC + size cap + backpressure), SQLite + Postgres outboxes with cross-backend parity, drainer, healthz, Helius driver (verify + normalize + catchup + admin) + captured-fixture suite + replay-parity gate. Peer transport to Geyser. |
| **02** | [Solana runtime](./02-solana-runtime.md) | 3-4 | `@ap3x/solana-signals`, `@ap3x/solana-strategy`, `@ap3x/solana-executor`, `@ap3x/solana-portfolio` — venue-agnostic runtime: signal ingestion, strategy dispatch with per-instance queues, Jito HTTP+gRPC submitters with failover, on-chain cost-basis reconstruction. Shipped 2026-04-19. |
| **02.5** | [Pump.fun protocol](./02.5-pumpfun-protocol.md) | 2-3 | `@ap3x/pumpfun-protocol` + `@ap3x/pumpfun-events` — first Solana vertical: decoders for both bonding curve + PumpSwap programs, bonding curve + AMM math, typed read-only on-chain client, pure instruction builders (`buildCreate` / `buildBuy` / `buildSell` / `buildPumpSwapSwap`) that PRP-03 wraps with safety rails. Off-chain APIs (advanced-api-v2 / frontend-api-v3) re-homed to PRP-04 as a signal source. |
...
- **02** = Solana runtime. Signals → strategy → executor → portfolio. Venue-agnostic so every future vertical (pump.fun, Raydium, Magic Eden) is a thin decoder + optional tx builder on top, not a re-implementation. Originally `PRP-02 pump.fun protocol`; that work moved to PRP-02.5 when the runtime's scope was promoted.
- **02.5 → 03 → 04** = pump.fun as the first full vertical on the substrate + runtime. Shipped vertically-integrated, validated end-to-end with one real strategy. Resist premature abstraction — build the specifics first.

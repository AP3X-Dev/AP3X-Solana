...
| **02.5** | [Pump.fun protocol](./02.5-pumpfun-protocol.md) | 1-2 | `@ap3x/pumpfun-*` — first Solana vertical: pump.fun program decoders, bonding curve math, typed read-only client, advanced-api-v2 compat wrapper (lifted from Chad's pump-gateway). Read-only; validates the substrate + runtime abstractions. |
